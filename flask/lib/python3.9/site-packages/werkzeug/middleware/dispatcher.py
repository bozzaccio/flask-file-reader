...
This example dispatches to an API src under "/api", an admin src
under "/admin", and an src that serves frontend files for all other
requests::

    src = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
